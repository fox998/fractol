...
static void print_fractl(t_window wind, t_rot_mat rot)
{
	t_complex	c;
	t_complex	z;
	double		temp;
	...
	while (++y < WIN_H)
	{
		...
		while (++x < WIN_W)
		{
			...
			while (z.i * z.i + z.r * z.r < 4.0 && num < 200)
			{
...
int main(void)
{
	...
	mlx_expose_hook(wind.win, &expose_hook, &rot);
	mlx_mouse_hook(wind.win, &mouse_hook, &rot);
	mlx_key_hook(wind.win, &key_hook, &rot);
	mlx_loop(wind.mlx);
}
